...
public class CompanyController extends BaseController {
    ...
    private String PREFIX = "/modular/base/company";
    ...
    public String index(Model model) {
        ...
        QueryWrapper<BCompanyInfo> query = new QueryWrapper<>();
        query.eq("userid",id);
        List<BCompanyInfo> list = bCompanyInfoService.list(query);
        if(list != null && list.size() == 1){
            model.addAttribute("companyfullname",list.get(0).getCompanyfullname());
            model.addAttribute("companyshortname",list.get(0).getCompanyshortname());
            model.addAttribute("projectfullname",list.get(0).getProjectfullname());
            model.addAttribute("projectshortname",list.get(0).getProjectshortname());
        }
    ...
    public ResponseData check() {
        ...
        return ResponseData.error("未绑定项目信息");
    }
    /**
     * 新增页面
     *
    ...
    @ResponseBody
    public ResponseData addItem(BCompanyInfo companyInfo) {
        companyInfo.setUserid(ShiroKit.getUser().getId());
        bCompanyInfoService.save(companyInfo);
        return ResponseData.success();
    }
    /**
     * 编辑页面
