layui.use(['table', 'admin', 'ax', 'func','form'], function () {
    ...
    Purchase.initColumn = function () {
        return [[
            ...
            {field: 'typeInDate', sort: true, title: '入库日期'},
    ...
    Purchase.openAddDlg = function () {
        func.open({
            title: '添加',
            content: Feng.ctxPath + '/storeIn/add',
    ...
    Purchase.review = function () {
        if (Purchase.check()) {
            ...
            var operation = function () {
                var ajax = new $ax(Feng.ctxPath + "/storeIn/review", function (data) {
                    if(data.code == 200){
    ...
    */
    Purchase.openEditDlg = function (data) {
            ...
            func.open({
                title: '修改',
                content: Feng.ctxPath + '/storeIn/edit?id=' + data.id,
                tableId: Purchase.tableId
            });
    };
    ...
    Purchase.onDeleteItem = function (data) {
            ...
            var operation = function () {
                var ajax = new $ax(Feng.ctxPath + "/storeIn/delete", function (data) {
                    Feng.success("删除成功!");
                    table.reload(Purchase.tableId);
                }, function (data) {
    ...
    Purchase.checkDetail = function (data) {
        func.open({
            title: '查看明细',
            content: Feng.ctxPath + '/storeIn/purchase_detail?id=' + data.id,
        });
    };
    ...
    Purchase.addDetail = function (data) {
        func.open({
            title: '录入明细',
            content: Feng.ctxPath + '/storeIn/purchase_detail_add?id='+data.id+'&purchaseno='+data.purchaseno,
        });
    };
    ...
    Purchase.printBar = function (data) {
        ...
        var operation = function () {
            layer.open({
                ...
                shadeClose: !0,
                area:["0px", "0px"],
                content: Feng.ctxPath +"/storeIn/prints/1"
            });
